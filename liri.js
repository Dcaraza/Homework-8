...
function runProgram() {
	activity('to start the program', 'node liri.js');
	...
	.then((answers) => {
		...
		switch (answers.program) {
			...
			case 'do-what-it-says':
				...
				activity('do-what-it-says  ', 'not supported');
				reRunProgram();
				break;

			default:
				activity('runProgram()  ', 'default Switch/Case');
...
function reRunProgram() {
	...
	.then((answers) => {
		if (answers.confirm) {
			activity('to reRun the program', 'Yes');
			runProgram();
		} else {
			activity('to abort the program', 'No');
			console.log("\nGood Bye!\n".cyan);
		}
	})
}
...
function getMusic(song) {
	...
	Spotify.search({ type: 'track', query: song, limit: 1 }, (err, data) => {
		...
		activity('spotify-this-song   ', song);
